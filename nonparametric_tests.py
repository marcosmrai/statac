...
def rank_miss(block):
    """
        Performs ranking on a observations block with missing values.
        
        Parameters
        ----------
        block : array_like
            A block of observations with np.nan on missing values.
            
        Returns
        -------
        rank: array_like
            The ranking of the block of observations.
        
        References
        ----------
        J. H. Skillings and G. A. Mack, On the Use of a Friedman-Type Statistic in Balanced and Unbalanced Block Designs, Technometrics, Vol. 23, No. 2 (May, 1981), pp. 171-177
        M. Chatfield and A. Mander, The Skillings–Mack test (Friedman test when there are missing data), The Stata Journal, Volume 9 Number 2: pp. 299-305
        
    """
    if sum(np.isnan(block)) in [len(block),len(block)+1]:
        raise('A block should have at least two observations')
    nobs = len(block) - sum(np.isnan(block))
    rank = [(nobs+1)/2 if np.isnan(block[i]) else r for i,r in enumerate(st.rankdata(block))]
    return rank
...
def skillings_test(*args):
    """
        Performs a Skillings–Mack test (Friedman test when there are missing data).
        Tests the hypothesis that in a set of k dependent samples groups (where k >= 2) at least two of the groups represent populations with different median values.
    ...
    """
    k = len(args)
    isvalue = np.array([1 - np.isnan(block) for block in zip(*args)])
    K = np.sum(isvalue,axis=1)
    n = len(args[0])
    ...
    rankings = np.array([rank_miss(block) for block in zip(*args)])
    rankings_avg = [sp.mean([case[j] for case in rankings]) for j in range(k)]
    rankings_cmp = [np.mean([r/sp.sqrt(k*(k+1)/(6.*n)) for r,ki in zip(treat,K)]) for treat in rankings.T]
    
    A = [sum([(r-(ki+1)/2)*np.sqrt((12./(ki+1))) for r,ki in zip(treat,K)]) for treat in rankings.T]
    
    L = isvalue.T@isvalue
    E = -np.copy(L)
    for i in range(k):
        E[i,i] = sum([L[i,j] for j in range(k) if i!=j])
    E11 = E[:-1,:-1]
    
    E11inv = np.linalg.inv(E11)
    sm = A[:-1]@E11inv@A[:-1]
    iman_davenport = ((n-1)*sm)/float((n*(k-1)-sm))
    p_value = 1 - st.f.cdf(iman_davenport, k-1, (k-1)*(n-1))
